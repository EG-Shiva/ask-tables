...
## Problem framing
Build a small web app where users upload CSV/Excel files and ask analytical questions in plain English. The risk with a naïve LLM demo is wrong numbers: models invent aggregates. I scoped a thin product — **AskTables** — that is correct first, then smart.
...
That split is the delta: AI proposes structure; deterministic code owns truth.

## Key decisions
- **Browser-first prototype** — Zero backend for the happy path; easy to demo in minutes.
- **Show the plan** — Reviewers can see what was understood vs executed.
...
- Server-side job queue + auth for a multi-user product.
- Eval set of questions with golden answers to regression-test the planner.

## Time box
Built as a focused prototype: complete upload → ask → chart loop, not a sprawling unfinished platform.
